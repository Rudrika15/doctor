  ...
  <form id="frm" action="{{route('hospitaltype.index')}}" method="get" class="mt-5">
    ...
    <div class="col-lg-4">
      <div class="form-group">
        <select name="typeName" class="search-dropdown form-control @error('typeName') is-invalid @enderror" placeholder="Enter Hospital Type">
          <option selected disabled>--Select HospitalType--</option>
        @foreach ($hospitaltype as $typeName)
          <option class="item text-center p-2 border" style="display: none;">{{$typeName->typeName}}</option>
          @endforeach
        </select>
        @error('cityName')
